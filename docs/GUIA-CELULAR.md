...
O coletor usa `/leagues` para validar competições e coverage, três buscas de fixtures por intervalo e uma busca em lote para detalhes de partidas prioritárias. Ele preserva o último JSON válido se uma chamada falhar ou a quota ficar baixa. O workflow **Update club and player profiles** roda uma vez por dia, atualiza até seis equipes, três perfis aprofundados de jogadores e listas de artilharia de até quatro competições. O plano completo foi calculado para ficar dentro de 100 requisições diárias.
