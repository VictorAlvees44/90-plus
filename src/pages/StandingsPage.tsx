...
export function StandingsPage() {
  ...
  return <main><PageHeader title="Tabelas" /><div className="league-tabs" role="tablist">{data.standings.map((item, index) => <button role="tab" aria-selected={index === selected} className={index === selected ? 'selected' : ''} key={item.league.id} onClick={() => setSelected(index)}>{item.league.name}</button>)}</div><section className="table-card"><h2>{standing.league.name}</h2><div className="standing-head"><span># Clube</span><span>J V E D GP GC SG P</span></div>{standing.rows.map(row => <div className="standing-row" key={row.team.id}><span><b>{row.rank}</b>{row.team.logo && <img src={imageSrc(row.team.logo)} alt="" />}{row.team.name}</span><span>{row.played} {row.win} {row.draw} {row.lose} {row.goalsFor} {row.goalsAgainst} {row.goalDiff} <strong>{row.points}</strong></span></div>)}</section></main>
}
